...
AShooterHUD::AShooterHUD(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	...
	JetpackIcon = UCanvas::MakeIcon(HUDJetpackTexture, 0, 0, 28, 28);
...
/** metodo creato sulla stessa falsariga di DrawHealth...che però serve a visualizzare l'energia del jetpack */
void AShooterHUD::DrawJetpackFuel()
{
	AShooterCharacter* MyPawn = Cast<AShooterCharacter>(GetOwningPawn());
	Canvas->SetDrawColor(FColor::White);
	const float JetpackPosX = ((Canvas->ClipX - JetpackBarBg.UL * ScaleUI) / 2);
	const float JetpackPosY = (Canvas->ClipY - (Offset + JetpackBarBg.VL) * ScaleUI) - 40;
	Canvas->DrawIcon(JetpackBarBg, JetpackPosX, JetpackPosY, ScaleUI);
	const float JetpackAmount = FMath::Min(1.0f, MyPawn->Fuel / MyPawn->GetMaxFuel());
...
/** metodo simile a quello che disegna l'alert per il basso livello di vita, però ho cambiato il colore da rosso a blu... vorrei anche aggiungre una iconcina che fa riferimento al gelo */
void AShooterHUD::DrawFreezed() {
	//Canvas->SetDrawColor(FColor::White);
	//const float FreezePosX =  Canvas->ClipX / 2;
	//const float FreezePosY = Canvas->ClipY / 2;
	
	AShooterCharacter* MyPawn = Cast<AShooterCharacter>(GetOwningPawn());
	const float AnimSpeedModifier = 1.0f + 5.0f;
	int32 EffectValue = 32 + 72;
	...
	FCanvasTileItem TileItem(FVector2D(0, 0), LowHealthOverlayTexture->Resource, FVector2D(Canvas->ClipX, Canvas->ClipY), FLinearColor(0.0f, 0.0f, 1.0f, AlphaValue));
	TileItem.BlendMode = SE_BLEND_Translucent;
	Canvas->DrawItem(TileItem);
...
void AShooterHUD::DrawHUD()
{
	...
	if(MyPawn && (MyPawn->IsFreezed() == true))
	{
		DrawFreezed();
	}
