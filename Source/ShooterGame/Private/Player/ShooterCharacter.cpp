...
bool AShooterCharacter::ServerSetJetpack_Validate(bool)
{
	return true;
}

void AShooterCharacter::ServerSetJetpack_Implementation(bool bNewJetpack)
{
...
bool AShooterCharacter::ServerSetFreeze_Validate(bool)
{
	return true;
}

void AShooterCharacter::ServerSetFreeze_Implementation(bool bNewFreeze)
{
	bIsFreezed = bNewFreeze;
}

void AShooterCharacter::StartFreeze()
{
		bIsFreezed = true;
		if (Role < ROLE_Authority)
		{
...
void AShooterCharacter::StopFreeze()
{
	bIsFreezed = false;
	if (Role < ROLE_Authority)
	{
...
void AShooterCharacter::HandleFreeze()
{
	AShooterCharacter::StartFreeze();
	FTimerHandle TimerHandle;
	GetWorldTimerManager().SetTimer(TimerHandle, this, &AShooterCharacter::StopFreeze, 2.0f);
}
...
{
	if ((Controller && Val != 0.f) && !IsFreezed())
	{
...
{
	AShooterCharacter::OnStartJump();
	AShooterCharacter::StartJetpack();
}

void AShooterCharacter::OnStopJetpack()
{
	AShooterCharacter::StopJetpack();
	AShooterCharacter::OnStopJump();
}

void AShooterCharacter::OnStartFire()
{
	AShooterPlayerController* MyPC = Cast<AShooterPlayerController>(Controller);
	if (MyPC && MyPC->IsGameInputAllowed() && !IsFreezed())
	{
		if (IsRunning())
		{
...
bool AShooterCharacter::IsFreezed() const
{
	return bIsFreezed;
}
...
void AShooterCharacter::GetLifetimeReplicatedProps(TArray< FLifetimeProperty > & OutLifetimeProps) const
{
	...
	DOREPLIFETIME_CONDITION(AShooterCharacter, bIsUsingJetpack, COND_SkipOwner);
	DOREPLIFETIME_CONDITION(AShooterCharacter, bIsFreezed, COND_SkipOwner);
...
float AShooterCharacter::GetJetpackSpeedModifier() const
{
	return JetpackSpeedModifier;
}

float AShooterCharacter::GetJetpackRefreshRate() const
{
	return JetpackRefreshRate;
}

bool AShooterCharacter::IsJetpackEnabled() const
{
	return bIsJetpackEnabled;
}
...
FVector AShooterCharacter::GetVectorForJump() {
	AShooterPlayerController* MyPC = Cast<AShooterPlayerController>(Controller);
	if (MyPC && MyPC->IsGameInputAllowed() && GetCharacterMovement()->IsMovingOnGround() == false)
	{
		// creo un fascio di 20 raggi che partono dal player per vedere quello più vicino dove verrà applicata la forza contraria per generare il salto sul muro
		...
		for (int i = 0; i < 20; i++)
		{
			float angle = 360 / 20 * i; // angolo dove mi trovo

			FVector Dir = Front * FMath::Sin(angle) + Side * FMath::Cos(angle);
			FVector End = Start + Dir * 200;
			DrawDebugLine(GetWorld(), Start, End, FColor::Black, true, 3);
			...
			if (Hit.bBlockingHit)
			{
				if ((Hit.Location - Start).Size() < DisMin)
				{
					HitLocation = Hit.Location;
					HitNormal = Hit.Normal;
					DisMin = (Hit.Location - Start).Size();
				}
			}
		}
...
void AShooterCharacter::WallJump()
{
	...
	if (Role < ROLE_Authority)
	{
		ServerAddForce(JumpVector);
	}
	LaunchCharacter(JumpVector, false, true);
}

bool AShooterCharacter::ServerAddForce_Validate(FVector force)
...
void AShooterCharacter::ServerAddForce_Implementation(FVector force)
{
